...
for i in range(num_datasets):
    ...
    alpha = rng.uniform(0.1, 0.9)  # Smoothing parameter
    beta = rng.uniform(0.1, 0.9)  # Trend parameter
    gamma = rng.uniform(0.1, 0.9)  # Seasonality parameter
    data_points = ExponentialSmoothing(rng.normal(scale=5, size=n)).fit(smoothing_level=alpha, smoothing_slope=beta, smoothing_seasonal=gamma).fittedvalues
